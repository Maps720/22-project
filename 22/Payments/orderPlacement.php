...
.error{
    ...
    $sql = "SELECT FlightDepartLocation, FlightArriveLocation, price FROM flights WHERE code =  $pay LIMIT 1";
    $result = mysqli_query($conn, $sql);
    if($result -> num_rows > 0){
        while($row = $result-> fetch_assoc()){
            ?> <h3>The price of flight code <?php echo $pay?> from <?php echo $row['FlightDepartLocation'];?> to <?php echo $row['FlightArriveLocation'];?>  is <?php echo $row['price'];?> </h3>
            <br>
    <div class="col__box">
